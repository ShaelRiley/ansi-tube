...
assert(content.includes('localResourceUrl("assets/cow-blond.png")'), "Cow PNG should load locally from the extension package");
assert(content.includes('globalCompositeOperation = "source-over"') && css.includes("mix-blend-mode: normal"), "The transparent cow asset should use palette-independent compositing in live view and export");
assert(fs.readFileSync(path.join(root, "assets/cow-blond.png")).includes(Buffer.from("tRNS")), "The cow PNG should contain transparency rather than depend on dark-palette blending");
...
assert(content.includes("scheduleCowAudioSuspend") && content.includes("suspendCowAudio"), "The cow AudioContext should not remain running between moos or after shutdown");
assert(content.includes("this.scheduleCowAudioSuspend((duration + 0.25) * 1000)"), "Cow audio should suspend shortly after its oscillators stop");
...
assert(content.includes("!this.hasAudioEffects()") && content.includes("this.bypassAudioGraph(graph)"), "Async audio graph setup should recheck current settings before enabling the wet path");
assert(content.includes("cancelAndHoldAtTime"), "Audio gain transitions should cancel stale automation without discontinuities");
assert(content.includes("const normalizedAudio =") && content.includes('["off", "bit1", "bit2"].includes'), "Persisted audio settings should be normalized before they can activate processing");
...
assert(content.includes("this.persistSettings({ panelCollapsed"), "Collapsed state should persist across videos");
assert(content.includes("readStoredSettings()") && content.includes("writeStoredSettings(values)"), "Storage access should tolerate an invalidated extension context");
assert(content.includes("Extension updated — refresh this YouTube tab to reconnect."), "Stale tabs should receive refresh guidance instead of an uncaught context error");
...
assert(!content.includes('this.panel.matches(":hover")'), "A stationary pointer should not prevent the normal-video compact bar from fading");
assert(content.includes('if (event.detail > 0) event.target.closest("button")?.blur()'), "Pointer collapse should release button focus while keyboard focus remains accessible");
